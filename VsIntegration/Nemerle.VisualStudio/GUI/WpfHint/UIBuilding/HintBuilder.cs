		...
		private static Inline GetInline(ParseToken token, Hint hint)
		{
			...
			if (token is ElementToken)
			{
				...
				switch (et.Name)
				{
					...
					case "u":
						return new Underline() { Cursor = Cursors.Hand }.Fill(et, hint);
					case "l":
						var res = new Span() { Cursor = Cursors.Hand }.Fill(et, hint);
						res.MouseEnter += (o, s) => { res.TextDecorations = TextDecorations.Underline; };
						res.MouseLeave += (o, s) => { res.TextDecorations = null; };
						return res;
					case "i":
					...
					case "code":
						return new Span { FontFamily = new FontFamily("Consolas")
										, FontSize = 12.0d * (96d / 72d)		}.Fill(et, hint);
					case "font":
